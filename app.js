...
function loadFromHistory(i) {
  try { const hist = JSON.parse(localStorage.getItem(HIST_KEY) || '[]'); restoreFormData(hist[i]); switchTab('form'); } catch(e) {}
}
...
function deleteFromHistory(i) {
  try { let hist = JSON.parse(localStorage.getItem(HIST_KEY) || '[]'); hist.splice(i, 1); localStorage.setItem(HIST_KEY, JSON.stringify(hist)); renderHistory(); } catch(e) {}
}
...
function initWordCount(id) {
  ...
  if (el.contentEditable === 'true') {
    ...
    el.addEventListener('input', () => {
      if (countWords(el.value) > WC_LIMIT) el.value = trimToWordLimit(el.value, WC_LIMIT);
      updateWordCount(id);
    });
  }
  updateWordCount(id);
...
function convertToRichEditor(id) {
  ...
  const extraTools = isAndrew ? '' : `
    <button type="button" class="rich-btn" title="Underline" onmousedown="event.preventDefault();document.execCommand('underline')"><u>U</u></button>
    <span class="rich-sep"></span>
    <button type="button" class="rich-btn" title="Numbered list" onmousedown="event.preventDefault();document.execCommand('insertOrderedList')">1. List</button>
    <button type="button" class="rich-btn" title="Clear formatting" onmousedown="event.preventDefault();document.execCommand('removeFormat')">
      <svg width="12" height="12" viewBox="0 0 14 14" fill="none" style="display:inline-block;vertical-align:middle"><path d="M2 3h10M5 3l1 8M9 3l-1 8M4 11h6" stroke="currentColor" stroke-width="1.2" stroke-linecap="round"/></svg>
    </button>
  `;
  toolbar.innerHTML = `
    <button type="button" class="rich-btn" title="Bold" onmousedown="event.preventDefault();document.execCommand('bold')"><b>B</b></button>
    <button type="button" class="rich-btn" title="Italic" onmousedown="event.preventDefault();document.execCommand('italic')"><i>I</i></button>
